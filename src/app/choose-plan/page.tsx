  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="text-center mb-12">
        ...
        <h2 className="text-4xl font-bold mb-4" style={{ fontFamily: "var(--font-display)" }}>Confirm Your Plan</h2>
        <p className="text-xl text-[var(--color-neutral-dark)]">Select the subscription that best suits your property.</p>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-8 mb-12">
        {plans.map((plan) => (
            ...
            >
              {selectedPlan === plan.id && (
                <div 
                  className="absolute -top-4 left-1/2 -translate-x-1/2 text-white px-4 py-1 rounded-full text-xs font-bold uppercase tracking-wider"
              ...
              <div className="mb-8">
                <h3 className="text-2xl font-bold mb-2" style={{ color: selectedPlan === plan.id ? plan.color : undefined }}>{plan.name}</h3>
              ...
              <div className="mb-8">
                <div className="flex items-baseline gap-1">
                  ...
                  <span className="text-[var(--color-neutral-dark)]">+ VAT {plan.period}</span>
                </div>
              </div>
                ...
                {plan.features.map((feature, i) => (
                  <li key={i} className="flex items-start gap-2">
                    <Check 
                      ...
                      style={{ color: selectedPlan === plan.id ? plan.color : "#9CA3AF" }}
                    />
        ...
        <div className="flex flex-col items-center gap-4">
          <Button 
            ...
            disabled={loading}
            size="lg"
            className="rounded-2xl px-12 py-7 text-xl font-bold text-white shadow-xl hover:shadow-2xl transition-all"
            style={{ background: plans.find(p => p.id === selectedPlan)?.color || "var(--color-accent-sage)" }}
          >
        ...
        <p className="text-sm text-[var(--color-neutral-dark)]">You'll be redirected to Stripe for secure payment processing.</p>
      </div>
    </div>
  );
}
