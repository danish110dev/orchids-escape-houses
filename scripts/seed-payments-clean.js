/**
 * Seed Payment Records Script (Corrected)
...
async function seedGuestBookingPayments() {
  ...
  for (const booking of allBookings.rows) {
    ...
    // Use first user from database as fallback
    const fallbackUserResult = await client.execute("SELECT id FROM user LIMIT 1");
    const userId = guestUserResult.rows.length > 0 
      ? guestUserResult.rows[0].id 
      : (fallbackUserResult.rows.length > 0 ? fallbackUserResult.rows[0].id : null);
    
    if (!userId) {
      console.log(`  ⚠️  Skipping booking #${booking.id} - no valid user found`);
      continue;
    }
